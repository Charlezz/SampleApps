...
public class Camera2Fragment extends Fragment
    ...
    public void onViewCreated(final View view, Bundle savedInstanceState) {
        ...
        ImageView box = (ImageView) view.findViewById(R.id.box);
        Point displaySize = new Point();
        getActivity().getWindowManager().getDefaultDisplay().getSize(displaySize);
        RelativeLayout.LayoutParams params = new RelativeLayout.LayoutParams(displaySize.x / 2, displaySize.y / 2);
        Log.e(TAG, "w:" + displaySize.x / 2 + "  y:" + displaySize.y / 2);
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        if (requestCode == REQUEST_CAMERA_PERMISSION) {
            if (grantResults.length != 1 || grantResults[0] != PackageManager.PERMISSION_GRANTED) {
                ErrorDialog.newInstance("no permission").show(getChildFragmentManager(), FRAGMENT_DIALOG);
            }
        } else {
    ...
    private void openCamera(int width, int height) {
        if (android.support.v4.content.ContextCompat.checkSelfPermission(getActivity(), Manifest.permission.CAMERA) != PackageManager.PERMISSION_GRANTED) {
